...
def test_live_catalog_composes_slack_permalink_into_approved_gmail_effect(tmp_path):
    slack = next(item for item in slack_definitions()
    ...
    compiled = PlanCompiler([slack, gmail], snapshots, "r1").compile({
        "tenant_id": "org:acme", "goal": "Send Laura the Slack permalink", "steps": [
            {"step_id": "read", "capability_id": slack.capability_id, "capability_version": slack.version, "connection_id": "conn:s", "descriptor_snapshot_hash": descriptor_hash(slack), "depends_on": [], "input": {"channel_id": "C1", "message_ts": "1.2"}},
            {"step_id": "email", "capability_id": gmail.capability_id, "capability_version": gmail.version, "connection_id": "conn:g", "descriptor_snapshot_hash": descriptor_hash(gmail), "depends_on": ["read"], "input": {"to": "laura@example.com", "subject": "Enlace", "body": {"$ref": "read.output.permalink"}}},
        ],
    })
    ...
    def dispatch(step, claim):
        ...
        if step["step_id"] == "read":
            ...
            return {"output": evidence, "receipt": evidence}
    ...
    assert dispatched[1][1] == {
        "to": "laura@example.com", "subject": "Enlace",
        "body": "https://acme.slack.com/archives/C1/p12",
    }
